...
**Milestone**: Establish scalable asset workflow and leverage open source assets

### 3.0.1 Sprite Atlas Pipeline
- [ ] Adopt a sprite atlas tool (e.g., TexturePacker / Free alternatives)
- [ ] Define atlas generation script and naming conventions
- [ ] Integrate atlas loading in Phaser preload step
- [ ] Document atlas update workflow for artists/devs
- [ ] Validate runtime memory/performance improvements

### 3.0.2 Open Source Asset Integration
- [ ] Identify high-quality OSS assets (Kenney, OpenGameArt, etc.)
- [ ] Verify licenses and attribution requirements
- [ ] Import placeholder sprites, UI elements, and SFX for rapid iteration
- [ ] Replace temporary assets incrementally with custom ones
- [ ] Maintain an assets manifest with sources and licenses

### 3.0.3 Font & UI Library Evaluation
- [ ] Select readable web fonts with OSS licenses
- [ ] Evaluate lightweight UI helpers (no heavy frameworks)
- [ ] Establish UI color/spacing/scale tokens for consistency
- [ ] Create reusable UI components for HUD and menus
- [ ] Document UI guidelines for future contributions
...
**Milestone**: Ship mobile-friendly on-screen controls and settings

### 3.1A.1 Touch Input Design & Settings
- [ ] Create on-screen control layout (virtual joystick + buttons)
- [ ] Provide settings: button size/position, left/right-hand swap
- [ ] Add haptics toggle and sensitivity sliders
- [ ] Implement accessibility options (color contrast, larger targets)

### 3.1A.2 Implementation
- [ ] Virtual joystick for rotation/aiming
- [ ] Fire button (maps to shooting)
- [ ] Thrust button (W equivalent)
- [ ] Afterburner button (Space equivalent) with hold behavior
- [ ] Pause/menu button
- [ ] Responsive layout across phone/tablet orientations

### 3.1A.3 Integration & Persistence
- [ ] Settings UI and localStorage persistence
- [ ] Tutorial overlays for first-time touch users
- [ ] Coexistence with desktop inputs (auto-detect device)

### 3.1A.4 Testing
- [ ] Test across Android/iOS browsers
- [ ] Verify no scroll/zoom gestures interfere
- [ ] Validate performance and input latency
- [ ] Ensure controls do not occlude critical HUD areas
