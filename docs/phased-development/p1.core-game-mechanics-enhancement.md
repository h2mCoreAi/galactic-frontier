...
- [ ] Create package.json with Vite, Phaser, and development dependencies
- [ ] Configure Vite development server on port 5174
- [ ] Set up Hot Module Replacement (HMR) for game files
- [ ] Configure Vite to serve from single-player/src/ directory
- [ ] Add Vite configuration for development and production builds

### 1.0.2 API Proxy Configuration
- [ ] Configure Vite dev server to proxy API calls to backend (port 3001)
- [ ] Set up proxy for /api/*, /auth/*, and /game-data/* routes
- [ ] Configure CORS handling for development environment
- [ ] Test API proxy functionality with mock backend responses
- [ ] Verify proxy works with config.json loading

### 1.0.3 Development Tooling Integration
- [ ] Configure Vite with TypeScript support for future migration
- [ ] Set up development server with automatic browser refresh
- [ ] Configure source maps for debugging
- [ ] Add development console logging and error overlays
- [ ] Test HMR functionality with game code changes

### 1.0.4 Build System Preparation
- [ ] Configure Vite production build output to dist/ directory
- [ ] Set up asset optimization and minification
- [ ] Configure build process for static file generation
- [ ] Test production build generation
- [ ] Verify built files work with nginx static serving

### 1.0.5 Development Environment Testing
- [ ] Test Vite dev server startup and hot reload functionality
- [ ] Verify game loads correctly in development environment
- [ ] Test API proxy with backend communication
- [ ] Validate development tools and debugging features
- [ ] Ensure development server doesn't conflict with nginx setup

### 1.0.6 PM2 Development Process Management
- [ ] Add PM2 ecosystem.dev.config.js with processes for vite (5174) and backend API (3001)
- [ ] Configure watch and autorestart for development processes
- [ ] Add npm scripts: pm2:dev:start, pm2:dev:restart, pm2:dev:stop, pm2:dev:logs
- [ ] Document developer workflow for starting/stopping dev environment
- [ ] Verify PM2 managed dev processes run cleanly and recover on changes
...
- [ ] Add config schema validation for required fields (define JSON Schema)
...
- [ ] Establish TypeScript baseline (tsconfig with strict true) for future migration
- [ ] Create minimal type definitions for Config, ShipConfig, EnemyConfig, ProjectileConfig
